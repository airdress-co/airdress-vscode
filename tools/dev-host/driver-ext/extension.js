...
function activate(context) {
  ...
  const tick = async () => {
    ...
    for (const f of files) {
      if (!f.startsWith("cmd-") || !f.endsWith(".json") || seen.has(f))
        continue;
      seen.add(f);
      ...
      try {
        if (payload.diagnostics) {
          fs.writeFileSync(
            out,
            JSON.stringify(
              vscode.languages.getDiagnostics().flatMap(([uri, ds]) =>
                ds.map((d) => ({
                  uri: uri.toString(),
                  message: d.message,
          ...
          fs.writeFileSync(
            ...
            JSON.stringify({
              ...
              extensions: vscode.extensions.all
                .filter((e) => e.id.startsWith("airdress"))
                .map((e) => ({ id: e.id, active: e.isActive })),
            }),
          );
        } else {
